...
import { getDelivery, deleteDelivery, cancelDelivery, getDeliverys } from '../../../api/order'
import { parseTime, cancel } from "../../../util/common";
import { useSelector } from 'react-redux';
import DeliveryDialog from '../../../components/order/DeliveryDialog';
...
export default function Delivery() {
  ...
  const handleShowModal = async (info) => {
    let res = await getDeliverys({id:info.id})
    setSelected(res[0]);
    setShowModal(true);
  }
  const columns = [
  ...
  const deleteO = async (info) => {
    let res = await deleteDelivery({ id: info.id, img_id: info.img_id })
    if (res.status) {
  ...
  const cancelOrder = async (id) => {
    let res = await cancelDelivery({ id: id })
    if (res.status) {
      message.success(res.message);
      fetchData();
    } else {
