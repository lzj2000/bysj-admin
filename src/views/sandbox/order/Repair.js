...
import { getRepair, cancelRepair, deleteRepair, getRepairs } from '../../../api/order'
import { parseTime, cancel } from "../../../util/common";
import { useSelector } from 'react-redux';
import RepairDialog from '../../../components/order/RepairDialog';
...
export default function Repair() {
  ...
  const handleShowModal = async (info) => {
    let res = await getRepairs({id:info.id})
    setSelected(res[0]);
    setShowModal(true);
  }
  const columns = [
  ...
  const deleteO = async (info) => {
    let res = await deleteRepair({ id: info.id,img_id: info.img_id })
    if (res.status) {
  ...
  const cancelOrder = async (id) => {
    let res = await cancelRepair({ id: id })
    if (res.status) {
      message.success(res.message);
      fetchData();
    } else {
