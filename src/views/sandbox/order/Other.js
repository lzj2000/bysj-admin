...
import { getOther, deleteOther, cancelOther, getOthers } from '../../../api/order'
import { parseTime, cancel } from "../../../util/common";
import { useSelector } from 'react-redux';
import OtherDialog from '../../../components/order/OtherDialog';
...
export default function Other() {
  ...
  const handleShowModal = async (info) => {
    let res = await getOthers({id:info.id})
    setSelected(res[0]);
    setShowModal(true);
  }
  const columns = [
  ...
  const deleteO = async (id) => {
    let res = await deleteOther({ id: id })
    if (res.status) {
  ...
  const cancelOrder = async (id) => {
    let res = await cancelOther({ id: id })
    if (res.status) {
      message.success(res.message);
      fetchData();
    } else {
